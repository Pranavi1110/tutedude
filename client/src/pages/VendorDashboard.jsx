...
const VendorDashboard = () => {
  ...
  const [orderStats, setOrderStats] = useState({ daily: [], weekly: [], monthly: [] });
  ...
  useEffect(() => {
    ...
    apiService.vendor.getOrderStats(user.id).then(setOrderStats).catch(() => setOrderStats({ daily: [], weekly: [], monthly: [] }));
  }, [user]);
  ...
  useEffect(() => {
    ...
    (async () => {
      let recData = await apiService.vendor.getRecommendedProducts(user.id).catch(() => []);
      console.log("Recommendations:", recData);
      let trendData = await apiService.vendor.getTrendingProducts().catch(() => []);
  ...
  if (loading) {
    return <div className="flex justify-center items-center h-64 text-xl">Loading products...</div>;
  }

  if (error) {
    return <div className="flex justify-center items-center h-64 text-red-500 text-xl">{error}</div>;
  }
  ...
  if (supplierFilter) {
    filteredProducts = filteredProducts.filter((p) => p.supplierId?.name === supplierFilter);
  }
      ...
      <div className="w-full max-w-6xl bg-transparent rounded-3xl shadow-2xl p-6 md:p-8 mt-4">
        ...
        <div className="mb-8">
          ...
          <div className="flex gap-2 mb-2">
            ...
            <button onClick={() => setOrderStatsType("monthly") } className={`px-3 py-1 rounded ${orderStatsType === "monthly" ? "bg-yellow-500 text-black" : "bg-gray-700 text-yellow-200"}`}>Month</button>
          </div>
          <div className="w-full h-64 bg-gray-900 rounded-xl p-4 flex items-center justify-center">
            {(orderStats[orderStatsType] && orderStats[orderStatsType].length > 0) ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={orderStats[orderStatsType]} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="_id" tick={{ fill: '#facc15', fontSize: 12 }} />
                  <YAxis tick={{ fill: '#facc15', fontSize: 12 }} allowDecimals={false} />
                  <Tooltip contentStyle={{ background: '#222', color: '#facc15' }} />
                  <Legend />
                  <Line type="monotone" dataKey="count" stroke="#facc15" strokeWidth={3} name="Orders" />
                </LineChart>
              </ResponsiveContainer>
            ) : (
              <span className="text-yellow-200 text-lg">No order data to display yet.</span>
            )}
          </div>
        </div>
        ...
        {loadingAI ? (
          ...
          <>
            ...
            {trending.length > 0 && (
              <div className="mb-8">
                <h3 className="text-xl font-bold text-pink-300 mb-2">Demand Spikes (Trending)</h3>
                <div className="flex flex-wrap gap-4">
                  {trending.map((p) => (
              ...
              />
                      ...
                      <button
                        className="mt-2 px-3 py-1 bg-pink-600  rounded hover:bg-pink-700"
                        onClick={() => addToCart(p)}
                      >Add to Cart</button>
                    </div>
                  ))}
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6 items-center w-full">
          <label className="text-white font-semibold" htmlFor="search-bar">Search:</label>
          <input
            id="search-bar"
            type="text"
          ...
          <select value={priceFilter} onChange={(e) => setPriceFilter(e.target.value)} className="px-4 py-2 rounded border border-blue-400 text-black">
            <option value="">Sort by Price</option>
            <option value="low-high">Low to High</option>
            <option value="high-low">High to Low</option>
          </select>
          <label className="text-white font-semibold">Supplier:</label>
          <select value={supplierFilter} onChange={(e) => setSupplierFilter(e.target.value)} className="px-4 py-2 rounded border border-blue-400 text-black">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            <h3 className="text-2xl font-bold mb-6 text-blue-200">Available Products</h3>
...
<div className="flex flex-row flex-wrap gap-2 mt-4 items-center">
  <button onClick={() => addToCart(p)} className="bg-gradient-to-r from-blue-700 via-purple-700 to-pink-700 text-white px-6 py-2 rounded-full">
    Add to Cart
  </button>
</div>
                  {/* Contact Supplier */}
            ...
            {totalPages > 1 && (
              <div className="flex justify-center mt-6 gap-2">
                <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="px-3 py-1 bg-blue-700 text-white rounded">Prev</button>
                {[...Array(totalPages)].map((_, idx) => (
                  <button key={idx + 1} onClick={() => handlePageChange(idx + 1)} className={`px-3 py-1 rounded ${currentPage === idx + 1 ? 'bg-pink-600 text-white' : 'bg-blue-200 text-blue-900'}`}>
                    {idx + 1}
                  </button>
                ))}
                <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="px-3 py-1 bg-blue-700 text-white rounded">Next</button>
              </div>
            )}
          </div>
          ...
          <div className="bg-gradient-to-br from-blue-900 via-purple-900 to-pink-900 p-6 rounded-2xl">
            ...
            {cart.length === 0 ? <p className="text-blue-300">No items</p> : (
              <div>
                {cart.map((item, idx) => (
                  <div key={idx} className="border-b border-blue-700 py-3 flex justify-between">
                    <div>
                      <p className="font-bold text-black">{item.name}</p>
                      <p className="text-sm text-black">{item.price}/{item.unit}</p>
                    </div>
                    <button
                      onClick={() => {
                      ...
                      className="bg-red-600 hover:bg-red-700 text-black px-3 py-1 rounded-full"
                    >Remove</button>
                  </div>
                ))}
                <button onClick={handlePlaceOrderClick} className="mt-6 w-full bg-gradient-to-r from-green-700 via-blue-700 to-purple-700 text-white py-3 rounded-full">
                  Place Order
                </button>
              </div>
            )}
